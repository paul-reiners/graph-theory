...
import com.leipzig48.graphs.erdos_renyi.ErdosRenyiGraph;
...
public class PathLengthsDFS {
    public static void main(String[] args) {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("data/out/random_simple_graph/path_lengths_dfs/stats.csv", true))) {
            ...
            for (int v = 1; v < upperV; v++) {
                for (int e = 0; e < upperE; e += 2) {
                    ...
                    for (int i = 0; i < 128; i++) {
                        RandomSimpleGraph randomSimpleGraph;
                        try {
                            randomSimpleGraph = new RandomSimpleGraph(v, e);
                        } catch (IllegalArgumentException excep) {
                            continue;
                        }
                        Graph graph = randomSimpleGraph.getGraph();
